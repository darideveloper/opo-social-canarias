...
export async function getUserByEmail(email: string) {
  try {
    const result = await query('SELECT * FROM auth_user WHERE email = $1', [email]);
...
export async function getProfileByUserId(userId: string) {
  try {
    const result = await query('SELECT * FROM jwt_auth_profile WHERE user_id = $1', [userId]);
    return result.rows[0] || null;
  } catch (error) {
...
export async function activateUser(email: string) {
  try {
    await query('UPDATE auth_user SET is_active = true WHERE email = $1', [email]);
